  ...
  while(1)
    {
      ...
      if (mag_init == 2 &&  magRaw[2] < mag_z_ref + 140 && magRaw[2] > mag_z_ref - 140 &&
	  magRaw[0] < mag_x_ref + 30 && magRaw[0] > mag_x_ref - 30 && a_y[0] > 16000)
      {
	...
	//fprintf(stderr,"\t\tM State: %d   M Timer: %d   M val: %d      R State: %d   R Timer: %d   R val: %d\n", melee_state, melee_timer, gesture_melee, reload_state, reload_timer, gesture_reload);
        ...
        print_counter = 0;
	if (a_x[0] < 900 && a_x[0] > -900 && a_y[0] < 900 && a_y[0] > -900 && g_x == 0 && g_y == 0 && g_z == 0)
	{
	    SEq_1 = 1;
	    SEq_2 = SEq_3 = SEq_4 = 0;
        }
